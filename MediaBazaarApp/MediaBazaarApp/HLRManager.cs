using System;
using System.Collections.Generic;
using System.Linq;
using System.Net;
using System.Net.Mail;
using System.Text;
using System.Threading.Tasks;
using System.Windows.Forms;

namespace MediaBazaarApp
{
    public class HLRManager
    {
        private IList<HolidayLeaveRequest> hlrs = new List<HolidayLeaveRequest>();
        private DBControl storage;

        public HLRManager()
        {
            hlrs = new List<HolidayLeaveRequest>();
            this.storage = new DBControl();
            this.LoadDataFromStorage();
        }

        public void LoadDataFromStorage()
        {
            this.hlrs = this.storage.GetHLRs();
        }

        public HolidayLeaveRequest GetHLR(int requestId)
        {
            foreach (HolidayLeaveRequest request in hlrs)
            {
                if (request.Id == requestId)
                {
                    return request;
                }
            }

            return null;
        }

        public IList<HolidayLeaveRequest> GetAllRequests()
        {
            LoadDataFromStorage();
            return hlrs;
        }

        public IList<HolidayLeaveRequest> GetAcceptedRequests()
        {
            return hlrs.Where(x => x.Status == "Accepted").ToList();
        }

        public IList<HolidayLeaveRequest> GetDeclinedRequests()
        {
            return hlrs.Where(x => x.Status == "Declined").ToList();
        }

        public IList<HolidayLeaveRequest> GetPendingRequests()
        {
            return hlrs.Where(x=> x.Status == "InProgress").ToList();
        }


        public void SendEmailToNotifyEmployee(string firstname, string email, HolidayLeaveRequest request, bool isRequestAccepted)
        {
            MailAddress to = new MailAddress(email);
            MailAddress from = new MailAddress("758b1e48c8-dc8967@inbox.mailtrap.io");

            MailMessage message = new MailMessage(from, to);
            string AcceptedRequestSubject = "Request for holiday leave accepted";
            string DeclinedRequestSubject = "Request for holiday leave declined";

            message.Subject = isRequestAccepted ? AcceptedRequestSubject : DeclinedRequestSubject;
            message.IsBodyHtml = true;

            string DeclinedRequestBody =
                $@"
                <p><span style='font-family: Verdana, Geneva, sans-serif;font-size:25px;'>Hello {firstname}</span></p>
                <p><span style='font-family: Verdana, Geneva, sans-serif;'>On <strong>{request.RequestDate}</strong> you submitted a request for a holiday leave starting from {request.StartDate.ToString("dd MMMM yyyy")} until {request.EndDate.ToString("dd MMMM yyyy")}.</span></p>
                <p><span style='font-family: Verdana, Geneva, sans-serif;'>We would like to let you know that the HR team has reviewed and <strong>declined&nbsp;</strong>your request.</span></p>
                <p><span style='font-family: Verdana, Geneva, sans-serif;'>If you believe that your request was wrongfully declined, feel free to contact the HR team by phone or email.</span></p>
                <p><span style='font-family: Verdana, Geneva, sans-serif;'><em><span style='font-size: 12px;'>This is an automated email; therefore, replying to it is not possible and if you do so, your email will not be received by us.</span></em></span></p>
                <p><span style='font-family: Verdana, Geneva, sans-serif;'>Kind regards,<br>Media Bazaar&apos;s team</span></p>
                <p><span style='font-family: Verdana, Geneva, sans-serif;'><img src='data:image/png;base64,'></span><br></p>
                ";
            string AcceptedRequestBody =
                $@"
                <p><span style='font-family: Verdana, Geneva, sans-serif;font-size:25px;'>Hello {firstname}</span></p>
                <p><span style='font-family: Verdana, Geneva, sans-serif;'>On <strong>{request.RequestDate}</strong> you submitted a request for a holiday leave starting from {request.StartDate.ToString("dd MMMM yyyy")} until {request.EndDate.ToString("dd MMMM yyyy")}.</span></p>
                <p><span style='font-family: Verdana, Geneva, sans-serif;'>We would like to let you know that the HR team has reviewed and <strong>accepted&nbsp;</strong>your request.</span></p>
                <p><span style='font-family: Verdana, Geneva, sans-serif;'><em><span style='font-size: 12px;'>This is an automated email; therefore, replying to it is not possible and if you do so, your email will not be received by us.</span></em></span></p>
                <p><span style='font-family: Verdana, Geneva, sans-serif;'>Kind regards,<br>Media Bazaar&apos;s team</span></p>
                <p><span style='font-family: Verdana, Geneva, sans-serif;'><img src='data:image/png;base64,'></span><br></p>
                ";
            message.Body = isRequestAccepted ? AcceptedRequestBody : DeclinedRequestBody;

            SmtpClient client = new SmtpClient("smtp.mailtrap.io", 2525)
            {
                Credentials = new NetworkCredential("f6ba1fe10a5e88", "e05fd608218765"),
                EnableSsl = true
            };

            try
            {
                client.Send(message);
            }
            catch (SmtpException ex)
            {
                MessageBox.Show(ex.Message);
            }
        }

        public void AcceptHLR(HolidayLeaveRequest request, DepartmentManagement departmentManagement)
        {
            // Accept - call update from db control
            // Load requests
            // Send email - proper msg
            request.Status = "Accepted";
            storage.UpdateHLR(request);
            this.LoadDataFromStorage();
            Employee receiver = departmentManagement.GetEmployeeById(request.EmployeeId);
            SendEmailToNotifyEmployee(receiver.FirstName, receiver.Email, request, true);
            
        }

        public void DeclineHLR(HolidayLeaveRequest request, DepartmentManagement departmentManagement)
        {
            // Decline - call update from db control
            // Load requests
            // Send email - proper msg
            request.Status = "Declined";
            storage.UpdateHLR(request);
            this.LoadDataFromStorage();
            Employee receiver = departmentManagement.GetEmployeeById(request.EmployeeId);
            SendEmailToNotifyEmployee(receiver.FirstName, receiver.Email, request, false);
        }

        public bool CheckForLeaveShiftPlanner(Employee emp, DateTime shiftDate)
        {
            foreach (HolidayLeaveRequest hlr in hlrs)
            {
                if (shiftDate >= hlr.StartDate && shiftDate <= hlr.EndDate && hlr.Status == "Accepted" && hlr.EmployeeId == emp.Id)
                {
                    return false; // shift can not be created for that day
                }
            }

            return true;
        }

    }
}
